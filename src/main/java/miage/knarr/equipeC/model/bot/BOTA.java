...
public class BOTA extends Joueur {
    
    public BOTA(String type, String nom, CouleurBateau couleurBateau, int pionsRecrue, int pionsBracelet, int pointVictoire, int pointRenomme) {
    ...
    public TypeAction choisirAction() {
        ...
        if (!actionsDisponibles.isEmpty()) {
            TypeAction action = actionsDisponibles.get(0);
            
            if (action == TypeAction.RECRUTER || action == TypeAction.EXPLORER) {
    ...
    public List<TypeAction> choisirActionsDuTour() {
        ...
        if (Math.random() < 0.5 && peutCommercer()) {
    ...
    public void recruter() {
        ...
        int randomIndex = random.nextInt(cartesVikingMain.size());
        CarteViking carteViking = cartesVikingMain.get(randomIndex);
        System.out.println(nom + " veut échanger sa carte : " + carteViking + " avec une carte Viking sur le plateau.");
        // Récupérer la carte depuis l'emplacement correspondant sur le plateau
        CarteViking nouvelleCarte = jeu.getPlateau().getEmplacementViking().get(carteViking.getCouleur());

        boolean aUtilisePionRecrue = false;
        VikingCouleur couleurEmplacement = null;
    
        if (nouvelleCarte != null) {
            if (pionsRecrue > 0 && random.nextBoolean()) {
                ...
                if (!cartesDisponibles.isEmpty()) {
                    int randomCarteIndex = random.nextInt(cartesDisponibles.size());
                    nouvelleCarte = cartesDisponibles.get(randomCarteIndex);
                    retirerPionsRecrue(1);
                    aUtilisePionRecrue = true;
                    System.out.println(nom + " a utilisé un pion recrue pour choisir une autre carte : " + nouvelleCarte);
                }
            }
            ...
            System.out.println("Aucune carte Viking disponible sur le plateau pour la couleur " + carteViking.getCouleur());
        }
    ...
    public void explorer() {
        ...
        CarteDestination destinationChoisie = cartesPossibles.get(
            new Random().nextInt(cartesPossibles.size())
        );
        ...
        if (acquerirCarteDestination(destinationChoisie)) {
            ...
            System.out.println(nom + " n'a pas pu acquérir la carte Destination: " + destinationChoisie.getId());
        }
    }
    ...
    public void commercer() {
        ...
        int nombreBraceletsUtilises = Math.min(pionsBracelet, 3);
    ...
    public boolean peutAcquerirCarteDestination(CarteDestination carteDestination) {
        ...
        for (Map.Entry<VikingCouleur, Integer> entry : requis.entrySet()) {
            ...
            if (couleur == VikingCouleur.COULEURANY) {
                // Gestion de COULEURANY : n'importe quelle couleur
                int totalVikings = 0;
                ...
                // Gestion de COULEURDIFF : couleurs différentes
    ...
    public boolean acquerirCarteDestination(CarteDestination carteDestination) {
        ...
        for (Map.Entry<VikingCouleur, Integer> entry : requis.entrySet()) {
            ...
            if (couleur == VikingCouleur.COULEURANY) {
                // Défausser n'importe quelle carte Viking jusqu'à atteindre la quantité requise
                ...
                for (VikingCouleur c : VikingCouleur.values()) {
                    if (c != VikingCouleur.COULEURANY && c != VikingCouleur.COULEURDIFF) {
                        ArrayList<CarteViking> equipageCouleur = zoneEquipage.getOrDefault(c, new ArrayList<>());
                        if (!equipageCouleur.isEmpty()) {
                ...
                // Parcourir les couleurs pour défausser les cartes
                for (VikingCouleur c : couleursPossedees) {
                    ...
                    if (cartesADefausser > 0) {
                        List<CarteViking> cartesASupprimer = new ArrayList<>(equipageCouleur.subList(0, cartesADefausser));
                ...
                if (quantiteRestante > 0) {
                    System.out.println("  → Utilisation de " + quantiteRestante + " pion(s) de recrutement pour compenser le manque");
                    pionsRecrueUtilises += quantiteRestante;
                }
    
            } else if (couleur == VikingCouleur.COULEURDIFF) {
                // Défausser une carte de chaque couleur différente jusqu'à atteindre la quantité requise
                int quantiteRestante = quantiteRequise;
    
                for (VikingCouleur c : VikingCouleur.values()) {
                    if (c != VikingCouleur.COULEURANY && c != VikingCouleur.COULEURDIFF && quantiteRestante > 0) {
                        ArrayList<CarteViking> equipageCouleur = zoneEquipage.getOrDefault(c, new ArrayList<>());
                        if (!equipageCouleur.isEmpty()) {
                ...
                if (quantiteRestante > 0) {
                    System.out.println("  → Utilisation de " + quantiteRestante + " pion(s) de recrutement pour compenser le manque en couleurs différentes");
                    pionsRecrueUtilises += quantiteRestante;
                }
    
            } else {
                // Gestion des couleurs spécifiques
                ArrayList<CarteViking> equipageCouleur = zoneEquipage.getOrDefault(couleur, new ArrayList<>());
                int quantiteDisponible = equipageCouleur.size();
